...
def evaluate_article(article, requirement, api_key=None, reference=None, model="deepseek-reasoner", use_AIGC=False):
    ...
    首先感受文章是否拥有一个真诚而独特的“灵魂”，也就是它的立意与情感是否真实可触，而非华丽辞藻下的空洞；
    接着审视它的“骨架”，即结构的流畅与逻辑的严谨，看它是否能自然引领我跟随作者的思路前行，而非生硬拼凑；
    随后品味它的语言是否精准与细节是否丰满，看是否有那么一个瞬间、一个画面或一句话让人心头一动或产生共鸣；
    最后评价“余味”，也就是它是否具有超越字面的思想深度或情感张力，能否在不经意间引发人们对生活、时代或人性更深一层的思考。
    评价时请务必结合上述四个维度进行综合评判，给出一个真实而有说服力的分数和反馈。"""
    prompt = f"文章要求：{requirement}\n\n文章内容：\n{article}\n\n请评价该文章，按照以下格式：\n分数：\n优点：\n缺点：\n改进建议："
    if reference:
        prompt += f"\n文章可能参考了以下内容，可以通过比对来确定真实性：\n{reference}"
    if use_AIGC:
        system_message += "\n特别注意：请仔细鉴别文章是否为AI生成，避免出现明显的AI痕迹，如公式化语言、缺乏个性、逻辑生硬等。如果发现有明显的AI生成痕迹，请在反馈中指出具体内容，并给予较低评分。"
    return call_deepseek(prompt, system_message, api_key, model)
